...
public class SolverService {
    public SolverResult solve(SolverRequest request) {
        ...
        return switch (request.getSolveMode()) {
            ...
            case ALL_SOLUTIONS_WITH_DEPTH -> buildAllSolutionsResult(request, solver.findAllSolutions(start, request.getGoal(), request.getMaxDepth()));
            case ALL_SOLUTIONS_UNBOUNDED -> buildAllSolutionsResult(request, solver.findAllPaths(start, request.getGoal()));
        };
    }
    ...
    private SolverResult buildShortestResult(SolverRequest request, AStar solver, State start) {
        ...
        return new SolverResult(request.getSolveMode(), request.getCapacities(), request.getStartVolumes(), shortestSolution, List.of());
    }

    private SolverResult buildAllSolutionsResult(SolverRequest request, List<List<Transfer>> solutions) {
        return new SolverResult(request.getSolveMode(), request.getCapacities(), request.getStartVolumes(), null, solutions);
    }
}
